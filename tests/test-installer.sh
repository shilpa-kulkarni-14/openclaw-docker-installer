...
# ── Test 8.5: Channel support ──
echo -e "\n${BOLD}[Channel Support]${RESET}"
assert_contains "Supports Slack" "slack:Slack" "$INSTALLER_CONTENT"
assert_contains "Supports Discord" "discord:Discord" "$INSTALLER_CONTENT"
assert_contains "Supports Telegram" "telegram:Telegram" "$INSTALLER_CONTENT"
assert_contains "Supports WhatsApp" "whatsapp:WhatsApp" "$INSTALLER_CONTENT"
assert_contains "Supports Microsoft Teams" "msteams:Microsoft Teams" "$INSTALLER_CONTENT"
assert_contains "Supports Google Chat" "google-chat:Google Chat" "$INSTALLER_CONTENT"
assert_contains "Supports Signal" "signal:Signal" "$INSTALLER_CONTENT"
assert_contains "Supports Matrix" "matrix:Matrix" "$INSTALLER_CONTENT"
assert_contains "Supports IRC" "irc:IRC" "$INSTALLER_CONTENT"
assert_contains "Supports Mattermost" "mattermost:Mattermost" "$INSTALLER_CONTENT"
assert_contains "Supports WebChat" "webchat:WebChat" "$INSTALLER_CONTENT"
assert_contains "Supports Twitch" "twitch:Twitch" "$INSTALLER_CONTENT"
assert_contains "Supports LINE" "line:LINE" "$INSTALLER_CONTENT"
assert_contains "Supports Feishu" "feishu:Feishu" "$INSTALLER_CONTENT"
assert_contains "Supports Nostr" "nostr:Nostr" "$INSTALLER_CONTENT"
assert_contains "Supports BlueBubbles" "bluebubbles:BlueBubbles" "$INSTALLER_CONTENT"
assert_contains "Supports macOS Native" "macos:macOS Native" "$INSTALLER_CONTENT"
assert_contains "Supports iOS/Android" "ios-android:iOS/Android" "$INSTALLER_CONTENT"
assert_contains "Channel selector function" "select_channels" "$INSTALLER_CONTENT"
