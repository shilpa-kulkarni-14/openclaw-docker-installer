...
This is the Docker version of the OpenClaw installer. It's designed so anyone — even someone who has never used a terminal before — can set up a working AI agent in under 5 minutes. If something goes wrong, the installer tells you exactly what happened and how to fix it.
...
## How It Compares to the Native Installer

| | Native (`install.sh`) | Docker (`docker-install.sh`) |
|---|---|---|
| **Installs on your machine** | Node.js, npm, jq | Nothing (everything in container) |
| **Credentials** | OS Keychain / encrypted files | `.env` file (chmod 600) |
| **Isolation** | Policy-based sandboxing | Full container isolation (12 layers) |
| **Auto-troubleshooting** | Error messages + log | 9 pre-flight checks + auto-fix + --doctor |
| **Best for** | Primary machine, long-term | Hackathons, experiments, shared machines |
